...
mydb=mysql.connector.connect(host='localhost',user='root',password='214209p',db='ecom')
...
def success(itemid,name,q,total):
    if session.get('user'):
        ...
        cursor.execute('select qyt from additems where item_id=uuid_to_bin(%s)',[itemid])
        count=cursor.fetchone()[0]
        uqyt=count-int(q)
        cursor.execute('update additems set qyt=%s where item_id=uuid_to_bin(%s)',[uqyt,itemid])
        mydb.commit()
        cursor.close()
